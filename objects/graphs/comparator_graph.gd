...
func set_header_width(value: float) -> void:
	...
	var sensor = _governor.get_sensor()
	var sensor_min = sensor.get_min()
	var sensor_max = sensor.get_max()
	_init_line_xy($GraphLine, value, _graph_y(sensor.get_value(), sensor_min, sensor_max, 0, 0))
	_init_line_xy($ErrorThreshold, value, _graph_y(_governor.error_threshold(), sensor_min, sensor_max, 0, 0))
	_init_line_xy($ErrorPeak, value, _graph_y(_governor.error_peak(), sensor_min, sensor_max, 0, 0))

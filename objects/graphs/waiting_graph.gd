class_name WaitingGraph extends Graph


func init (header_frame: Array[String]) -> void:
	header_frame.append("waiting_value")
	header_frame.append("waiting_max")
...
func set_header_width(value: float) -> void:
	...
	_init_label_x($WaitingMax, value - $WaitingMax.size.x - TEXT_MARGIN)
	_init_line_x($StartLine, value, true)
	_init_line_x($WaitingLine, value, false)
